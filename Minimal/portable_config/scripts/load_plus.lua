...
function find_and_add_entries()
	...
	if opt.level == 0 then
		msg.verbose("Automatic queue abort: feature disabled")
		return
	elseif #dir == 0 then
		msg.warn("Automatic queue abort: non-local path")
		return
	end

	pl_count = mp.get_property_number("playlist-count", 1)
	if pl_count > 1 then
		msg.warn("Automatic queue abort: playlists have been created/modified manually")
		return
	end
	...
	if files == nil then
		msg.info("Auto-queue: no other files in current directory")
		return
	end
	...
	msg.trace("Auto-queue: the sequence the current file is in "..current)
	...
	for direction = -1, 1, 2 do -- 2 iterations, with direction = -1 and +1
		for i = 1, opt.max_entries do
			...
			if not filenames[file] then
				if direction == -1 then
					msg.info("Auto Queue Append (before)）" .. file)
					table.insert(append[-1], 1, {filepath, pos - 1})
				else
					msg.info("Auto Queue Append (after)" .. file)
					if pl_count > 1 then
...
function import_url()
	...
	local res = utils.subprocess({
		args = {'powershell', '-NoProfile', '-Command', [[& {
			...
			$u8 = [System.Text.Encoding]::UTF8
			$out = [Console]::OpenStandardOutput()
			$urlname = [Microsoft.VisualBasic.Interaction]::InputBox("Enter Address", "Open URL", "https://")
